...
Web Workbench 的任务输入框支持显式 Skill 引用。输入 `$` 会弹出当前进程已加载的 Skills，可继续输入名称筛选，并使用上下键、`Enter`、`Tab`、`Esc` 或鼠标完成选择。选中的 Skill 在输入框中显示为蓝色引用，提交时转换为规范的 `$<skill-name>` 文本；Agent 会先调用 `load_skill`，实际加载过程可在执行记录中查看。
